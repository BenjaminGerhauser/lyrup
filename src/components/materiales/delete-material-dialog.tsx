...
import { deleteMaterial, type MaterialActionResult } from '@/app/actions/materials'
...
export function DeleteMaterialDialog({ materialId, materialName }: DeleteMaterialDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button variant="destructive" size="sm" aria-label={`Eliminar ${materialName}`} />
        }
      >
        <Trash2Icon />
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ¿Estás seguro que querés eliminar <strong>{materialName}</strong>? Esta acción no se puede deshacer.
          </DialogDescription>
        </DialogHeader>
